...
import { Camera, Upload, X, Check, RotateCcw, Trash2 } from 'lucide-react';
...
export default function PickupPhotoCapture({ onComplete, onCancel, pickupRef }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    if (photo) {
      try {
        ...
        const fileName = `pickup-${pickupRef}-${Date.now()}.jpg`;
    ...
    <div className="fixed inset-0 bg-black/90 z-50 flex flex-col">
      <div className="bg-orange-500 text-white p-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-bold">Proof of Pickup</h2>
        </div>
      ...
      <div className="flex-1 flex flex-col p-4 overflow-auto">
        <p className="text-white/80 text-center mb-4">
          Take a photo of the empty dumpster location
        </p>
        ...
        {photo ? (
          ...
          <div className="flex-1 flex flex-col gap-4 justify-center">
            ...
            <Button onClick={() => cameraInputRef.current?.click()}
              className="h-20 text-xl font-bold bg-orange-500 hover:bg-orange-600">
        ...
        <div className="mt-4">
          ...
          <Textarea value={notes} onChange={(e) => setNotes(e.target.value)}
            placeholder="Location cleared, area clean, etc."
      ...
      <div className="p-4 bg-white border-t">
        <Button onClick={handleSubmit} disabled={uploading}
          className="w-full h-16 text-xl font-bold bg-orange-500 hover:bg-orange-600">
          {uploading ? 'Uploading...' : <><Check className="w-6 h-6 mr-2" /> Confirm Pickup</>}
        </Button>
      </div>
    </div>
  );
